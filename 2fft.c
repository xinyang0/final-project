...
void InitTDAndFD(double *&TD_real,double *&TD_imag,double *&FD_real,double *&FD_imag,unsigned char *data,int lWidth,int lHeight)
{
    ...
    for(int i = 0;i < w * h; i++)
    {
        TmpFD_real[i*w+j]=0.0;
        TmpFD_imag[i*w+j]=0.0;
    }
    ...
    for(int k=0;k<r;k++)
    {
        for(int j=0;j<pow(2,k);j++)
        {
            for(int i=0;i<pow(2,(r-k-1));i++)
            {
                ...
                X2_real[i+p+(int)(pow(2,(r-k-1)))]=(X1_real[i+p]-X1_real[i+p+(int)(pow(2,(r-k-1)))]) * W_real[i*pow(2,k)]-(X1_imag[i+p]-X1_imag[i+p+(int)(pow(2,(r-k-1)))]) * W_imag[i*pow(2,k)];
                X2_imag[i+p+(int)(pow(2,(r-k-1)))]=(X1_real[i+p]-X1_real[i+p+(int)(pow(2,(r-k-1)))]) * W_imag[i*pow(2,k)]+(X1_imag[i+p]-X1_imag[i+p+(int)(pow(2,(r-k-1)))]) * W_real[i*pow(2,k)];
            }
        }
        X_real=X1_real;
        X_imag=X1_imag;
...
void FFT_2D(double *&TD_real, double *&TD_imag, double *&FD_real, double *&FD_imag, int lWidth,int lHeight)
{
    ...
    TmpTD_real = calloc(h, sizeof(double));
    TmpTD_imag = calloc(h, sizeof(double));
    TmpFD_real = calloc(h, sizeof(double));
    TmpFD_imag = calloc(h, sizeof(double));
    ...
    for(int i=0;i<w;i++)
    {
        ...
        for(int j=0;j<h;j++)
        {
            TmpTD_real[j]=TD_real[j*w+i];
            TmpTD_imag[j]=TD_imag[j*w+i];
        }
        FFT_1D(TmpTD_real, TmpTD_imag, TmpFD_real, TmpFD_imag, lHeight);
        
        for(int j=0;j<h;j++)
        {
            TD_real[j*w+i]=TmpFD_real[j];
            TD_imag[j*w+i]=TmpFD_imag[j];
        }
    }
    ...
    for(int i=0;i<h;i++)
    {
        ...
        FFT_1D(TmpTD_real, TmpTD_imag, TmpFD_real, TmpFD_imag, lWidth);
        
        for(int j=0;j<w;j++)
        {
            FD_real[i*w+j]=TmpFD_real[j];
            FD_imag[i*w+j]=TmpFD_imag[j];
        }
    }
